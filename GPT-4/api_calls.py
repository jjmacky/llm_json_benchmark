...
def serialize_content_block(chat_completion):
    # Initialize an empty dictionary to capture various pieces of information
    serialized = {}

    # Capture the 'id', 'created', 'model', and other root-level attributes of ChatCompletion
    serialized['id'] = getattr(chat_completion, 'id', 'N/A')
    serialized['created'] = getattr(chat_completion, 'created', 'N/A')
    serialized['model'] = getattr(chat_completion, 'model', 'N/A')
    serialized['object'] = getattr(chat_completion, 'object', 'N/A')

    # Check if there are choices and specifically capture the first choice's details
    if chat_completion.choices:
        first_choice = chat_completion.choices[0]
        serialized['choice'] = {
            'finish_reason': getattr(first_choice, 'finish_reason', 'N/A'),
            'index': getattr(first_choice, 'index', 'N/A')
        }

        # Capture the content of the message, which is the raw JSON string you're attempting to parse
        message_content = getattr(first_choice.message, 'content', 'No content available')
        serialized['choice']['message_content'] = message_content

    else:
        serialized['choice'] = 'No choices available'
    ...
    for index, row in df.iterrows():
        ...
        for instruction_version, instruction_text in instructions_array.items():
            ...
            try: 
                ...
                unsuccessful_content_blocks.append(serialize_content_block(completion))
